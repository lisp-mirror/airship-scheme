...
;;; Note: A functional library can only use things that are
;;; functional, i.e. things that have no side effects. Use of things
;;; that are not functional is an error.
;;;
;;; SRFI 172 functional serves as a base for other functional
;;; libraries by preventing the import of things that would be an
;;; error to use.
(define-library (srfi 172 functional)
  ...
  (export - * / + < <= = => > >= abs acos and angle append apply asin assoc assq
          ...
          caddar cadddr caddr cadr call/cc call-with-current-continuation
          ...
          char-upper-case? char-whitespace? complex? cond cond-expand cons cos
          ...
          inexact? infinite? integer? integer->char lambda lcm length let let*
          let*-values letrec letrec* let-values list list? list->string
          list->vector list-copy list-ref list-tail log magnitude
          ...
          modulo nan? negative? newline not null? number? number->string
          numerator odd? open-input-bytevector open-input-string
          open-output-bytevector open-output-string or pair? parameterize
          ...
          reverse round sin sqrt square string string? string<? string<=?
          string=? string>? string>=? string->list string->number string->utf8
          ...
          substring symbol? symbol=? symbol->string tan truncate truncate/
          ...
          vector-ref when with-exception-handler zero?))
